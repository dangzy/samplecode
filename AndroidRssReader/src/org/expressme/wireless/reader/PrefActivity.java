...
public class PrefActivity extends Activity {
    ...
    private final int freqscale = 10;
    ...
    void savePreferences() {
		...
		boolean shownetimg = ((RadioButton) findViewById(R.id.radio_show_image)).isChecked();
        int freq = (((Spinner) findViewById(R.id.spinner_freq)).getSelectedItemPosition())*10;
        int expires = ((Spinner) findViewById(R.id.spinner_expires)).getSelectedItemPosition() + 1;
        serviceBinder.storePreferences(unreadOnly, shownetimg, freq, expires);
    }
    ...
    void loadPreferences() {
		...
		boolean shownetimg = serviceBinder.getPreferenceOfImage();
		RadioButton imgradio = (RadioButton) findViewById(shownetimg ? R.id.radio_show_image : R.id.radio_block_image);
        imgradio.setChecked(true);

        int freq = serviceBinder.getPreferenceOfFreq();
        log.info("freq = " + freq);
        String[] freq_array = new String[ReadingService.FREQ_MAX/freqscale];
        String unit_minute = getResources().getString(R.string.unit_minute);
        freq_array[0]= getResources().getString(R.string.pref_fetch_stop);
        for (int i=1; i<freq_array.length; i++) {
            freq_array[i] = i*freqscale + " " + unit_minute;
        }
        ...
        spinner_freq.setSelection(freq/freqscale);
