...
public class ReadingService extends Service {
    ...
    public static final int FREQ_DEFAULT = 30;
	...
	public boolean getPreferenceOfImage() {
        SharedPreferences pref = getSharedPreferences(PREF_NAME, MODE_PRIVATE);
        return pref.getBoolean(PREF_IMAGE, true);
    }

    public void storePreferences(boolean unreadOnly, boolean shownetimg, int freq, int expires) {
		...
		editor.putBoolean(PREF_IMAGE, shownetimg);
     ...
     */
    public void refreshFeeds(String feed) {
      if (feed == null){
    ...
    private void refreshEachFeeds(final String url) {
	 if (null == url)
	 	return;
        new Thread() {
            public void run() {
                ...
                fetcher.fetch(
                    ...
                    new FeedFetcherListenerAdapter() {
                            ...
                            public void onFetched(String feedUrl, String charset, byte[] content) {
                                ...
                                try {
									try{
										InputSource insrc=new InputSource(new ByteArrayInputStream(content));
                                    	FeedParser.getDefault().parse(insrc, listener);
										insrc.getByteStream().close();
									}catch(Exception e){
										InputSource insrc=new InputSource(new InputStreamReader(new ByteArrayInputStream(content),"GBK"));
										FeedParser.getDefault().parse(insrc, listener);
										insrc.getCharacterStream().close();
									}
    ...
    private Cursor findSubscriptionUrlByFreq() {
        try {
        	g_cursor = getContentResolver().query(SubscriptionColumns.URI, new String[] { SubscriptionColumns.URL }, null, null, SubscriptionColumns.FREQUENCY + "," + SubscriptionColumns._ID + " desc");
            if (g_cursor.moveToFirst()) {
    ...
    public void loadStoredItem() {
        ...
        try {
            String where = ItemColumns.PUBLISHED + "<" + expiredTime + " AND " + ItemColumns.UNREAD + "=0";
            cursor = getContentResolver().query(ItemColumns.URI, new String[] { SubscriptionColumns.URL }, null, null, SubscriptionColumns.FREQUENCY + "," + SubscriptionColumns._ID + " desc");
        }
        catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void markAllItemsRead() {
        ContentValues values = new ContentValues();
        values.put(ItemColumns.UNREAD, 0);
        getContentResolver().update(ItemColumns.URI, values, null, null);
    }

	public void delAllItems() {
        getContentResolver().delete(ItemColumns.URI, null, null);
		loadStoredItem();
		ContentValues cv = new ContentValues();
    ...
    public void onStart(Intent intent, int startId) {
		...
		delayed=getPreferenceOfFreq()*ONE_MINUTE;
		expires=getPreferenceOfExpires()*ONE_WEEK;
		triggerNextTimer();
    }
